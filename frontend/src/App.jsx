...
const translations = {
  en: {
    ...
    locationPlaceholder: "Location (e.g., Free / Bangalore)",
    ...
    verifying: "Verifying...",
    verified: "Verified ✓",
    idNotice: "Enter a valid ID number for check.",
  ...
  hi: {
    ...
    locationPlaceholder: "स्थान (जैसे फ्री / बैंगलोर)",
    ...
    hoursLabel: "घंटे",
    phoneLabel: "फोन",
    updateTitle: "🔄स्थिति अद्यतन",
    updateBtn: "स्थिति अद्यतन करें"
  },
  te: {
    title: "లేబర్ కనెక్ట్",
    ...
    locationPlaceholder: "ప్రాంతం (ఉదా. ఫ్రీ / బెంగళూరు)",
    ...
    hoursLabel: "గంటలు",
    phoneLabel: "ఫోన్",
    updateTitle: "🔄స్థితి నవీకరణ",
    updateBtn: "స్థితిని నవీకరించండి"
  }
};
...
const DynamicLocation = ({ locationText, targetLang, defaultText }) => {
  ...
  useEffect(() => {
    let isMounted = true;
    if (!locationText || targetLang === 'en') {
      setTranslated(locationText);
      return;
    }
...
function App() {
  ...
  const handleSubmitWorker = (e) => {
    ...
    // Auto-derive availability if location contains "free" (case-insensitive)
    const isFree = formData.location.trim().toLowerCase().includes('free');
  ...
  return (
    <div style={styles.container}>
      ...
      <section style={{ ...styles.registrationBlock, marginTop: '20px' }}>
        <h2 style={styles.sectionTitle}>{t.updateTitle}</h2>
        <form onSubmit={handleStatusUpdate} style={styles.form}>
          ...
          {updateMsg && <p style={{ color: '#2b6cb0', marginTop: '10px', fontWeight: 'bold' }}>{updateMsg}</p>}
        </form>
      </section>

      {/* AVAILABLE WORKERS DIRECTORY */}
        ...
        {loading ? (
          ...
          <div style={styles.grid}>
            {filteredWorkers.map((worker) => {
              // Determine if worker is free based on location text or isAvailable boolean flag
              const isFree = (worker.location && worker.location.toLowerCase().includes('free')) || worker.isAvailable;
                  ...
                  <div style={styles.cardHeader}>
                    <div>
                      ...
                      <span style={{
                        ...
                        backgroundColor: isFree ? '#e8f5e9' : '#ffebee'
                      }}>
                      ...
                      {worker.isVerified && (
                        <div style={{ marginTop: '2px' }}>
                          <span style={styles.verifiedBadge}>{t.verifiedWorker}</span>
                        </div>
                      )}
                    </div>
...
//         fetchWorkers(); // Refresh the list automatically
//       })
